...
def evaluate_models(X_train, y_train, cv_folds):
    ...
    for model_name in ["Logistic Regression", "Random Forest Classifier"]:
...
def intervention_suggestions(row, prediction):
    ...
    attendance = row.get("AttendanceRate", np.nan)
    study_hours = row.get("StudyHoursPerWeek", np.nan)
    support = str(row.get("ParentalSupport", "")).strip().lower()
    previous_grade = row.get("PreviousGrade", np.nan)
    online_classes = row.get("Online Classes Taken", np.nan)
...
with tab1:
    ...
    st.write(
        ...
        "classification models for education analytics."
    )
...
with tab2:
    ...
    if st.button("Run Model Comparison"):
        ...
        ax_cmp.set_title("Cross-Validated F1 Score by Model")
...
with tab3:
    ...
    if "pipeline" not in st.session_state:
        ...
        st.write(
            ...
            "students who may benefit from support, and as an interpretable machine learning "
            "study of the factors associated with academic outcomes."
        )
