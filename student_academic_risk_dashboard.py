...
def fit_predict_model(model_name, X_train, y_train, X_test):
    ...
    return pipeline, y_pred, label_encoder
...
def evaluate_models(X_train, y_train, cv_folds):
    ...
    for model_name in get_available_models().keys():
        try:
            ...
            if model_name == "XGBoost Classifier":
                label_encoder = LabelEncoder()
                y_train_for_cv = label_encoder.fit_transform(y_train)
            else:
            ...
            if np.all(np.isnan(scores["test_accuracy"])):
                results.append(
                    {
                        "Model": model_name,
                        "CV Accuracy Mean": np.nan,
                        "CV Accuracy Std": np.nan,
                        "CV Precision Mean": np.nan,
                        "CV Recall Mean": np.nan,
                        "CV F1 Mean": np.nan,
                        "Status": "Failed during cross-validation",
                    }
                )
            else:
                results.append(
                    {
                        ...
                        "CV Precision Mean": np.nanmean(scores["test_precision_macro"]),
                        "CV Recall Mean": np.nanmean(scores["test_recall_macro"]),
                        "CV F1 Mean": np.nanmean(scores["test_f1_macro"]),
                        "Status": "OK",
                    }
                )
        except Exception as e:
    ...
    ok_df = comparison_df[comparison_df["Status"] == "OK"].sort_values("CV F1 Mean", ascending=False)
    failed_df = comparison_df[comparison_df["Status"] != "OK"]

    if not failed_df.empty:
        comparison_df = pd.concat([ok_df, failed_df], ignore_index=True)
    else:
        comparison_df = ok_df.reset_index(drop=True)
...
with tab2:
    ...
    st.write(
        ...
        "Models that fail are skipped and reported instead of crashing the app."
    )
    ...
    if st.button("Run Model Comparison"):
        ...
        ok_models_df = comparison_df[comparison_df["Status"] == "OK"].copy()

        if ok_models_df.empty:
            st.error("All models failed during cross-validation. Please try a different dataset or settings.")
        else:
            fig_cmp, ax_cmp = plt.subplots(figsize=(10, 5))
            ax_cmp.bar(ok_models_df["Model"], ok_models_df["CV F1 Mean"])
            ax_cmp.set_title("Cross-Validated Macro F1 Score by Model")
            ax_cmp.set_ylabel("Macro F1")
            ax_cmp.tick_params(axis="x", rotation=35)
            st.pyplot(fig_cmp)

            chosen_model = ok_models_df.iloc[0]["Model"]
            st.info(f"Best cross-validated model by Macro F1: {chosen_model}")

            pipeline, y_pred, label_encoder = fit_predict_model(
                selected_model, X_train_cv, y_train_cv, X_test_holdout
            )
    ...
    if "pipeline" not in st.session_state:
        ...
        if st.button("Predict Student Outcome"):
            ...
            if label_encoder is not None:
                pred = label_encoder.inverse_transform([int(pred_raw)])[0]
            else:
                pred = pred_raw
    ...
    if "pipeline" not in st.session_state:
        ...
        if batch_file is not None:
            ...
            if missing_cols:
                ...
                batch_preds_raw = pipeline.predict(batch_input)
                if label_encoder is not None:
                    batch_preds = label_encoder.inverse_transform(batch_preds_raw.astype(int))
                else:
                    batch_preds = batch_preds_raw
